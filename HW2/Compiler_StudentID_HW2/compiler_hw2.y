...
print_content
	:STR_CONST{}
	|ID{
		if(lookup_symbol($1,"variable")==0 && lookup_global($1,"variable")==0){
			print_sym_flag = 2;
...
declaration
	:type_specifier init_declarator SEMICOLON{
		if(func_declare_flag == 1){
			...
			scope_flag--;
			kill_symbol();
			if(lookup_symbol($2,"function") == 0 && lookup_global($2,"function")==0){
				insert_symbol($2,$1,"function",attribute_buf);
				strcpy(attribute_buf,"");
			}
			else{
...
primary_expression
	:ID{
		$$=$1;
		if(lookup_global($1,"variable") == 0 && lookup_symbol($1,"variable" )==0){
			print_sym_flag = 2;
			strcat(message_buf,"Undeclared Variable ");
			strcat(message_buf,$1);
		}
